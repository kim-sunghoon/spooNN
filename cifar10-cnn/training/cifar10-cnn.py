    ...
    def _build_graph(self, inputs):
        ...
        with remap_variables(new_get_variable), \
            ...
            logits = (LinearWrap(image).apply(monitor, 'image_out')
                      ...
                      .Conv2D('conv3', out_channel=128)
                      .apply(fg)
                      .BatchNorm('bn3').apply(activate).apply(monitor, 'conv3_out')
                      .Conv2D('conv4', out_channel=128)
                      .apply(fg)
                      .BatchNorm('bn4').apply(activate).apply(monitor, 'conv4_out')
                      .Conv2D('conv5', out_channel=128)
                      .apply(fg)
                      .BatchNorm('bn5').apply(activate).apply(monitor, 'conv4_out')
                      .FullyConnected('fc0', out_dim=1024 + 512, use_bias=False, activation=tf.nn.relu).apply(monitor, 'fc0_out')
                      .FullyConnected('fc1', use_bias=False, out_dim=512, activation=tf.nn.relu).apply(monitor, 'fc1_out')
                      .FullyConnected('fc2', use_bias=False, out_dim=self.cifar_classnum, nl=tf.identity).apply(monitor, 'fc2_out')
                      ())
...
def run_test(weights_file, test_file, classnum):
    if MONITOR == 1:
        monitor_names = ['image_out', 'conv0_out', 'pool0_out', 'conv1_out', 'conv2_out', 'pool1_out', 'conv3_out', 'fc0_out', 'fc1_out', 'fc2_out']
    else:
        monitor_names = []
    output_names = ['prob']
