    ...
    def _build_graph(self, inputs):
        ...
        with remap_variables(new_get_variable), \
            ...
            logits = (LinearWrap(image).apply(monitor, 'image_out')
                      ...
                      .apply(fg)
                      .BatchNorm('bn1_1')
                      .apply(activate).apply(monitor, 'conv1_1_out')
                      .Conv2D('conv1_2', 64)
                      .apply(fg)
                      .BatchNorm('bn1_2')
                      .apply(activate).apply(monitor, 'conv1_2_out')
                      .MaxPooling('pool1', 2).apply(monitor, 'pool1_out')
                      # 112
                      .Conv2D('conv2_1', 128)
                      .apply(fg)
                      .BatchNorm('bn2_1')
                      .apply(activate).apply(monitor, 'conv2_1_out')
                      .Conv2D('conv2_2', 128)
                      .apply(fg)
                      .BatchNorm('bn2_2')
                      .apply(activate).apply(monitor, 'conv2_2_out')
                      .MaxPooling('pool2', 2).apply(monitor, 'pool2_out')
                      # 56
                      .Conv2D('conv3_1', 256)
                      .apply(fg)
                      .BatchNorm('bn3_1')
                      .apply(activate).apply(monitor, 'conv3_1_out')
                      .Conv2D('conv3_2', 256)
                      .apply(fg)
                      .BatchNorm('bn3_2')
                      .apply(activate).apply(monitor, 'conv3_2_out')
                      .Conv2D('conv3_3', 256)
                      .apply(fg)
                      .BatchNorm('bn3_3')
                      .apply(activate).apply(monitor, 'conv3_3_out')
                      .MaxPooling('pool3', 2).apply(monitor, 'pool3_out')
                      # 28
                      .Conv2D('conv4_1', 512)
                      .apply(fg)
                      .BatchNorm('bn4_1')
                      .apply(activate).apply(monitor, 'conv4_1_out')
                      .Conv2D('conv4_2', 512)
                      .apply(fg)
                      .BatchNorm('bn4_2')
                      .apply(activate).apply(monitor, 'conv4_2_out')
                      .Conv2D('conv4_3', 512)
                      .apply(fg)
                      .BatchNorm('bn4_3')
                      .apply(activate).apply(monitor, 'conv4_3_out')
                      .MaxPooling('pool4', 2).apply(monitor, 'pool4_out')
                      # 14
                      .Conv2D('conv5_1', 512)
                      .apply(fg)
                      .BatchNorm('bn5_1')
                      .apply(activate).apply(monitor, 'conv5_1_out')
                      .Conv2D('conv5_2', 512)
                      .apply(fg)
                      .BatchNorm('bn5_2')
                      .apply(activate).apply(monitor, 'conv5_2_out')
                      .Conv2D('conv5_3', 512)
                      .apply(fg)
                      .BatchNorm('bn5_3')
                      .apply(activate).apply(monitor, 'conv5_3_out')
                      .MaxPooling('pool5', 2).apply(monitor, 'pool5_out')
                      .FullyConnected('fc6', use_bias=False, out_dim=512).apply(activate).apply(monitor, 'fc6_out')
                      .FullyConnected('fc7', use_bias=False, out_dim=512).apply(activate).apply(monitor, 'fc7_out')
                      .FullyConnected('fc8', use_bias=False, out_dim=self.cifar_classnum, nl=tf.identity).apply(monitor, 'fc8_out')
                      ())
...
def run_test(weights_file, test_file, classnum):
    if MONITOR == 1:
        monitor_names = ['image_out', 'conv1_1_out', 'conv1_2_out', 'pool1_out', 'conv2_1_out', 'conv2_2_out', 'pool2_out', 
                'conv3_1_out', 'conv3_2_out', 'conv3_3_out', 'pool3_out','conv4_1_out', 'conv4_2_out', 'conv4_3_out', 'pool4_out', 'conv5_1_out', 'conv5_2_out', 'conv5_3_out', 
                'fc6_out', 'fc7_out', 'fc8_out']
    else:
        monitor_names = []
    output_names = ['prob']
...
def dump_weights(meta, model, output):
    ...
    with tf.Graph().as_default() as G:
        ...
        with sess.as_default():
            ...
            target_FMpS = 300000
            non_quantized_layers = ['conv1_1/Conv2D', 'fc8/MatMul']
            ...
            generateConfig(layers_list, 'cifar100-cnn-config-{}W{}A.h'.format(BITW, BITA))
            genereateHLSparams(layers_list, network_model, 'cifar100-cnn-params-{}W{}A.h'.format(BITW, BITA), fw)
