...
def get_data():
    ...
    dataset_train = RandomMixData([d1, d2])
    dataset_test = dataset.SVHNDigit('test', shuffle=False)

    dataset_train = BatchData(dataset_train, 256)
...
def run_test(weights_file, test_file, classnum):
    ...
    l = loader()
    l.load_svhn(test_file, num_samples=NUM)
...
def dump_weights(meta, model, output):
    ...
    with tf.Graph().as_default() as G:
        ...
        with sess.as_default():
            ...
            generateConfig(layers_list, 'svhn-cnn-config-{}W{}A.h'.format(BITW, BITA))
            genereateHLSparams(layers_list, network_model, 'svhn-cnn-params-{}W{}A.h'.format(BITW, BITA), fw)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--testfile', help='testfile from test dataset')
    parser.add_argument('--output', help='output for dumping')
    parser.add_argument('--classnum', help='10 for svhn dataset',
                        type=int, default = 10)
